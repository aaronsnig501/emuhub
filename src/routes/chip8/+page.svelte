<script lang="ts">
	...
	import { Chip8, type Chip8Snapshot } from '$lib/chip8';
	...
	const currentTheme = $derived(
		displayThemes.find((theme) => theme.id === currentThemeId) ?? displayThemes[0]
	);
	const activeKeys = $derived.by(() => {
		frameVersion;
		return chip8.keypad.map((pressed, index) => ({ pressed, label: keypadLabels[index] }));
	});
	const registerPairs = $derived.by(() => {
		frameVersion;
		return Array.from(chip8.registers).map((value, index) => ({
	...
	const executionStats = $derived.by(() => {
		frameVersion;
		return [
			{ label: 'PC', value: `0x${chip8.programCounter.toString(16).toUpperCase().padStart(3, '0')}` },
			{ label: 'I', value: `0x${chip8.indexRegister.toString(16).toUpperCase().padStart(3, '0')}` },
			{ label: 'Delay', value: chip8.delayTimer.toString() },
			{ label: 'Sound', value: chip8.soundTimer.toString() }
		];
	});
	...
	function saveState() {
		...
		const nextSlot: SaveSlot = {
			...
			programCounter: `0x${chip8.programCounter.toString(16).toUpperCase().padStart(3, '0')}`,
...
<svelte:head>
	<title>EMU·HUB — CHIP-8 Player</title>
	<meta
		name="description"
		content="Play CHIP-8 ROMs in the browser with emulator controls, save states, ROM details, and display theme selection."
	/>
</svelte:head>

<svelte:window onkeydown={onKeyDown} onkeyup={onKeyUp} />

<div class="min-h-screen bg-[#090909] text-[#f0f0f0]">
	<div class="mx-auto max-w-7xl px-4 pb-10 pt-8 sm:px-6 lg:px-8">
		<div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
			<div>
				<p class="section-eyebrow">CHIP-8 player</p>
				<h1 class="mt-4 font-display text-[clamp(3.5rem,8vw,5.5rem)] leading-[0.9] tracking-[0.16em]">
					PLAY. PAUSE.
					<br />
					<span class="text-[#7dff96]">TUNE THE MACHINE.</span>
				</h1>
				<p class="mt-4 max-w-2xl text-sm leading-7 text-[#a8a8a8] sm:text-base">
					Load a local ROM, control execution, capture save states, and inspect the emulator while it
					runs. The page is built around the real CHIP-8 core, not a static mock.
				</p>
			</div>

			<label
				class="inline-flex cursor-pointer items-center gap-3 rounded-xl border border-white/12 bg-[#151515] px-4 py-3 font-mono text-[0.72rem] uppercase tracking-[0.18em] text-[#d6d6d6] hover:border-white/25 hover:text-white"
			>
				<span>Load ROM</span>
				...
				<div class="overflow-hidden rounded-[1.75rem] border border-white/12 bg-[#141414] shadow-[0_30px_80px_rgba(0,0,0,0.45)]">
					...
					<div class="grid gap-4 px-4 py-4 lg:grid-cols-[1.4fr_1fr]">
						<div class="flex flex-wrap items-center gap-2">
							<button
								type="button"
								class="rounded-md bg-[#ff3030] px-4 py-2 font-mono text-[0.68rem] uppercase tracking-[0.18em] text-white shadow-[0_0_24px_rgba(255,48,48,0.35)] hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
								onclick={() => (isRunning ? pauseLoop() : startLoop())}
								disabled={!hasLoadedRom}
							>
								{isRunning ? 'Pause' : 'Run'}
							</button>
							<button
								type="button"
								class="rounded-md border border-white/16 px-4 py-2 font-mono text-[0.68rem] uppercase tracking-[0.18em] text-[#d7d7d7] hover:border-white/30 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
								onclick={restart}
								disabled={!hasLoadedRom}
							>
								Restart
						...
						<div class="rounded-2xl border border-white/8 bg-[#101010] px-4 py-3">
							...
							<div class="flex flex-wrap gap-2">
								{#each SPEED_OPTIONS as option}
									<button
										type="button"
										class={`rounded-md px-3 py-2 font-mono text-[0.66rem] uppercase tracking-[0.16em] ${
											speedMultiplier === option
												? 'bg-[#7dff96] text-[#08100b]'
												: 'border border-white/14 text-[#b5b5b5] hover:border-white/28 hover:text-white'
										}`}
										onclick={() => (speedMultiplier = option)}
									>
										{option}x
									</button>
								{/each}
						...
						<div class="grid gap-3 sm:grid-cols-2">
							{#each displayThemes as theme}
								<button
									...
									class={`rounded-2xl border p-4 text-left ${
										currentThemeId === theme.id
											? 'border-white/28 bg-[#1c1c1c]'
											: 'border-white/10 bg-[#171717] hover:border-white/22'
									}`}
									onclick={() => {
										currentThemeId = theme.id;
										render();
								...
								>
									...
									<div class="mt-1 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-[#8c8c8c]">
										Display + glow palette
									</div>
								</button>
							{/each}
						</div>
			...
			<aside class="space-y-4">
				<section class="rounded-[1.5rem] border border-white/12 bg-[#141414] p-5">
					...
					{#if saveStates.length === 0}
						<div class="rounded-2xl border border-dashed border-white/12 bg-[#171717] px-4 py-5 text-sm leading-7 text-[#9a9a9a]">
							Save states appear here after you load a ROM and capture a snapshot.
						</div>
					{:else}
						<div class="space-y-3">
								...
								<div class="rounded-2xl border border-white/8 bg-[#1a1a1a] p-4">
									<div class="flex items-start justify-between gap-3">
										<div>
											...
											<div class="mt-2 font-mono text-[0.62rem] uppercase tracking-[0.16em] text-[#7dff96]">
												{slot.romName}
											</div>
										</div>
										<div class="font-mono text-[0.62rem] uppercase tracking-[0.16em] text-[#666666]">
											{slot.createdAt}
										</div>
									</div>
									<div class="mt-4 grid gap-2 sm:grid-cols-3">
										<div class="rounded-xl bg-[#111111] px-3 py-2">
											<div class="font-mono text-[0.58rem] uppercase tracking-[0.16em] text-[#666666]">PC</div>
											<div class="mt-1 font-mono text-[0.72rem] text-[#e6e6e6]">{slot.programCounter}</div>
										</div>
										<div class="rounded-xl bg-[#111111] px-3 py-2">
											<div class="font-mono text-[0.58rem] uppercase tracking-[0.16em] text-[#666666]">Delay</div>
											<div class="mt-1 font-mono text-[0.72rem] text-[#e6e6e6]">{slot.delayTimer}</div>
										</div>
										<div class="rounded-xl bg-[#111111] px-3 py-2">
											<div class="font-mono text-[0.58rem] uppercase tracking-[0.16em] text-[#666666]">Sound</div>
											<div class="mt-1 font-mono text-[0.72rem] text-[#e6e6e6]">{slot.soundTimer}</div>
										</div>
									</div>
					...
					<div class="space-y-3">
						{#each keymapRows as row}
							<div class="rounded-2xl border border-white/8 bg-[#1a1a1a] px-4 py-4">
								...
								<div class="mt-3 font-mono text-[0.7rem] uppercase tracking-[0.18em] text-[#7dff96]">
									Keyboard: {row.keys}
								</div>
							</div>
						{/each}
				...
				<section class="rounded-[1.5rem] border border-white/12 bg-[#141414] p-5">
					...
					<div class="mb-4 grid grid-cols-4 gap-2">
						{#each activeKeys as key}
							<div
								class={`rounded-xl border px-0 py-3 text-center font-mono text-[0.72rem] uppercase tracking-[0.16em] ${
									key.pressed
										? 'border-[#7dff96]/40 bg-[#7dff96]/12 text-[#7dff96]'
										: 'border-white/8 bg-[#1a1a1a] text-[#7d7d7d]'
								}`}
							>
								{key.label}
							</div>
